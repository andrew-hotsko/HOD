...
// Semantic CSS variable helper
export const V = (name) => `var(--${name})`;

// ── Label (eyebrow/meta text) ─────────────────────────────
export function HodLabel({ children, style, color = 'bone-faint' }) {
  return (
...
// ── Tag / chip (selectable) ───────────────────────────────
export function HodTag({ children, selected = false, onClick, compact = false, style }) {
...
// ── Primary button ────────────────────────────────────────
export function HodButton({ children, onClick, variant = 'primary', size = 'md', full = false, disabled = false, style }) {
  const sizes = {
...
// ── Horizontal rule ───────────────────────────────────────
export function HodRule({ ticks = false, color = 'iron-700', style }) {
  if (!ticks) {
...
// ── Stat block ────────────────────────────────────────────
export function HodStat({ label, value, unit, align = 'left', accent = false, size = 'md' }) {
  const sizes = { sm: 20, md: 32, lg: 56, xl: 92 };
  return (
...
export function HodReg({ size = 10, color = 'iron-600', style }) {
  return (
...
// ── HOD brand mark ────────────────────────────────────────
export function HodMark({ size = 'md', showDate = true, date, accent = true }) {
  const sizes = {
