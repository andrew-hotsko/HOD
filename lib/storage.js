...
const K = {
  ...
  quotesOff:  'hod.quotesOff',   // '1' when the user has silenced motivational quotes
};
...
// ── Quotes on/off toggle ─────────────────────────────────
export function areQuotesEnabled() {
  if (typeof localStorage === 'undefined') return true;
  return localStorage.getItem(K.quotesOff) !== '1';
}

export function setQuotesEnabled(enabled) {
  if (typeof localStorage === 'undefined') return;
  if (enabled) localStorage.removeItem(K.quotesOff);
  else localStorage.setItem(K.quotesOff, '1');
}
