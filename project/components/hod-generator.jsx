// HOD — workout generator
// Pulls from the actual garage inventory. Deterministic by seed (so each date
// produces the same workout across reloads). Returns a structured plan the
// Live screen can render adaptively.

const GARAGE = {
  rack: 'Rogue Monster Lite squat rack',
  barbell: { name: 'Barbell', plates: [10, 25, 35, 45] }, // per side pairs
...
// Simple seedable PRNG so "today" is stable.
...
// Movement library, tagged by equipment + pattern.
const MOVES = [
  // Barbell
...
// Build a workout
function generateHOD({ date = new Date(), intensity = 'HARD', style = 'CROSSFIT', duration = 30 } = {}) {
  ...
  // Pick a warmup block (always present, fixed shape)
  const warmup = {
    label: 'WARMUP',
    duration: 5,
  ...
  if (format === 'AMRAP') {
    ...
    main = {
      ...
      items: moves.map((m, i) => ({
        name: m.name,
        reps: pick(rand, [8, 10, 12, 15])[0] || 10,
        unit: m.unit,
    ...
    main = {
      ...
      items: [
        ...
        ...accessory.map(m => ({ name: m.name, reps: pick(rand, [[10],[12]])[0], unit: m.unit, load: suggestLoad(m, intense), accessory: true })),
      ],
    };
  }
  ...
  const finisher = rand() > 0.5 ? {
    label: 'FINISHER',
    duration: 3,
    note: pick(rand, ['3 min Assault Bike — max cal', '50 burpees for time', '2 min plank accumulation', 'Death by push-ups — 5 min cap']),
  } : null;
...
Object.assign(window, { GARAGE, MOVES, STYLES, INTENSITIES, DURATIONS, generateHOD });
